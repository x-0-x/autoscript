...
detectdistro() {
        ...
        if [[ -n ${os_release} ]]; then
          ...
          if [[ -n ${distrib_id} ]]; then
            if [[ ${BASH_VERSINFO[0]} -ge 4 ]]; then
              ...
              distrib_id=$(for i in ${distrib_id}; do
                FIRST_LETTER=$(echo -n "${i:0:1}" | tr "[:lower:]" "[:upper:]")
                echo -n "${FIRST_LETTER}${i:1} "
              done)
              distro=${distrib_id% }
              unset distrib_id
            fi
  ...
  if grep -q -i 'Microsoft' /proc/version 2>/dev/null ||
    grep -q -i 'Microsoft' /proc/sys/kernel/osrelease 2>/dev/null; then
...
detectshell() {
  if [[ ! "${shell_type}" ]]; then
    if [[ "${distro}" == "Cygwin" || "${distro}" == "Msys" || "${distro}" == "Haiku" || "${distro}" == "Alpine Linux" || "${distro}" == "Mac OS X" || "${distro}" == "macOS" || "${distro}" == "TinyCore" || "${distro}" == "Raspbian" || "${OSTYPE}" == "gnu" ]]; then
    ...
    if ps -U "${USER}" | grep -q -i 'finder'; then
      if [[ ${TERM_PROGRAM} == "iTerm.app" ]] && [ -f ~/Library/Preferences/com.googlecode.iterm2.plist ]; then
        ...
        for i in $(seq 0 $((${#iterm2_theme_info[*]} / 2 - 1))); do
          found_uuid=$(
            str1=${iterm2_theme_info[$i * 2]}
            echo "${str1}"
          )
          if [[ $found_uuid == $iterm2_theme_info ]]; then
            gtkFont=$(
              str2=${iterm2_theme_info[$i * 2 + 1]}
              echo ${str2:25:${#str2}-25-2} | sed 's/ [0-9]*$//'
            )
            break
          fi
        done
...
takeShot() {
  if [[ -n "$screenCommand" ]]; then
    ...
    if [ "$distro" == "Mac OS X" || "$distro" == "macOS" ]; then
      ...
      for ((i = 2; i <= displays; i++)); do
        shotfiles[$i]="$(echo ${shotfile} | sed "s/\(.*\)\./\1_${i}./")"
      done
      printf "Taking shot in 3.. "
      sleep 1
      printf "2.. "
      sleep 1
      printf "1.. "
...
infoDisplay() {
  ...
  if [[ "${distro}" == "Android" ]]; then
    myhostname=$(echo -e "${labelcolor} ${hostname}")
    out_array=("${out_array[@]}" "$myhostname")
    mydistro=$(echo -e "$labelcolor OS:$textcolor $distro $distro_ver")
    out_array=("${out_array[@]}" "$mydistro")
    mydevice=$(echo -e "$labelcolor Device:$textcolor $device")
    out_array=("${out_array[@]}" "$mydevice")
    myrom=$(echo -e "$labelcolor ROM:$textcolor $rom")
    out_array=("${out_array[@]}" "$myrom")
    mybaseband=$(echo -e "$labelcolor Baseband:$textcolor $baseband")
    out_array=("${out_array[@]}" "$mybaseband")
    ...
    mygpu=$(echo -e "$labelcolor GPU:$textcolor $cpu")
